...
public class MessageProxyDAO extends ProxyDAO<Message> implements DAO<Message> {
    ...
    public List<Message> doRetrieveByCondition(String condition, int offset, int row_count) throws SQLException {
        ...
        logger.log(Level.INFO, "MessageProxyDAO:doRetrieveByCondition() - condition: " + condition + " LIMIT " + offset + ", " + row_count);

        List<Message> messages = GenericProxyDAO.genericProxyDoRetrieveByCondition(condition, offset, row_count, dao, out);
