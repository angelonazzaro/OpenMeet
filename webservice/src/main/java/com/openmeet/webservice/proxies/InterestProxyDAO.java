...
public class InterestProxyDAO extends ProxyDAO<Interest> implements DAO<Interest> {
    ...
    public List<Interest> doRetrieveByCondition(String condition, int offset, int rows_count) throws SQLException {
        ...
        logger.log(Level.INFO, "InterestProxyDAO:doRetrieveByCondition() - condition: " + condition + " LIMIT " + offset + ", " + rows_count);

        List<Interest> interests = GenericProxyDAO.genericProxyDoRetrieveByCondition(condition, offset, rows_count, dao, out);
