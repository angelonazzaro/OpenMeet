       ...
       // Get Data
        ...
        try {
            data = qjx.doRetrivedByJoin(String.format(
                    "SELECT %s.*, CONCAT(%s.meeterName, ' ', %s.meeterSurname) AS meeterfullName, %s.email FROM %s JOIN " +
                            "%s ON %s.id = %s.meeterReported WHERE %s.isArchived = false",
                    Report.REPORT,  Meeter.MEETER, Meeter.MEETER, Meeter.MEETER, Report.REPORT,
                    Meeter.MEETER,  Meeter.MEETER, Report.REPORT, Report.REPORT));
