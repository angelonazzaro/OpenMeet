...
class MessageProxyDAO(context: Context) : ContextDAO(context), DAO<Message> {
    ...
    override fun doRetrieveByCondition(condition: String, rows_count: Int): MutableList<Message>? {
    ...
    override fun doRetrieveByCondition(condition: String, offset: Int, rows_count: Int): MutableList<Message>? {
    ...
    override fun doRetrieveByKey(key: String): Message? {
        ...
        val message = jsonResp.getString("data")
        val gson = GsonBuilder().setDateFormat("yyyy-MM-dd").create()

        DAO.logger.log(Level.INFO, "doRetrieveByKey: $message")
    ...
    override fun doRetrieveAll(): MutableList<Message>? {
        ...
        DAO.logger.log(Level.INFO, "doRetrieveAll: $messages")
        ...
        VolleyRequestSender.getInstance(this.context)
            .doHttpPostRequest(getUrl() + "MessageService",
                hashMapOf(
                    ...
                    "message" to GsonBuilder().setDateFormat("yyyy-MM-dd").create().toJson(obj)
                ),
                object : VolleyResponseCallback {
                    override fun onError(error: String) {
                        resp = error
